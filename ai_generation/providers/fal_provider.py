"""
FAL.ai — Fast inference API with Flux and other models.
https://fal.ai
"""
import asyncio
...
class FalProvider(ImageProvider):
    """FAL.ai image generation — fast Flux inference."""
    ...
    capabilities = [
        ProviderCapability(
            name="text_to_image",
            description="Generate images via FAL.ai",
            input_types=["text"],
    ...
    async def generate_image(
        ...
        if not api_key:
            return GenerationResult(
                ...
                request_id=request_id, error="No FAL_KEY set",
        ...
        payload = {
            "prompt": prompt,
            "image_size": {"width": min(width, 1024), "height": min(height, 1024)},
            "num_images": 1,
        }
        if negative_prompt:
